...
class Lingua
    ...
    def initialize
        ...
        @LinguaParser = Parser.new("LinguaNotitia") do
            ...
            rule :for_loop do
                match('for', '(', :declaration, :bool_expression, ';', :assignment, ')', '{', :blocks, '}', ';'){|_, _, var, bool, _, assignment, _, _, blocks,_, _| For_loop_node.new(var, bool, assignment, blocks)}
            end
            ...
            rule :declaration do
                ...
                varName, nil) }
                match('@', :datatype, :varName, '=', :expression, ';') {|_,datatype, varName, _, expression,_| DeclareVar.new(datatype,
                varName, expression, true) }
                match('@', :datatype, :varName, '=', :varName, ';') {|_,datatype, varName, _, expression,_| DeclareVar.new(datatype,
                varName, expression, true) }
                match('@', :datatype, :varName, ';') {|_,datatype, varName,_| DeclareVar.new(datatype,
                varName, nil, true) }
            end
            ...
            rule :varName do
                match(/[A-z]+[_A-z0-9]*/) {|m| Find_Variable.new(m)}
...
if ARGV[0]
    ...
    if test.openFile(ARGV[0])    
         puts "Ran file #{ARGV[0]}"
    else
        puts "Couldn't run file #{ARGV[0]}"
    end
